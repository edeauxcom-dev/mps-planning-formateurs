...
// Date pivot : avant = ancien schéma (jeudi regroupement),
// à partir de = nouveau schéma (dernier L/M/M présentiel par TP)
export const DATE_NOUVEAU_SCHEMA = new Date(2026, 9, 1); // 1er octobre 2026
...
export async function getAccessToken(): Promise<string> {
  ...
  if (!data.access_token) throw new Error("Échec de l'authentification Google : " + JSON.stringify(data));
...
async function sheetsFetch(path: string, init: RequestInit = {}) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(`Erreur API Google Sheets (${res.status}) : ${JSON.stringify(data)}`);
  return data;
}
